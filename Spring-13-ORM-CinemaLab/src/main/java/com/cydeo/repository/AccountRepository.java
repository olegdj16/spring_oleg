...
//04.26.2023 vid 2, @23:46
    ...
    List<Account> findAllByAndOrderByAgeDesc();
    ...
    List<Account> getAccountAndSortByAge(Integer age);
    ...
    @Query(value = "SELECT * FROM account_details WHERE name ILIKE concat('%',?1,'%')" +
            "OR lower(address) ILIKE lower(concar('%',?1,'%'))" + //makes sense insensitive
            "OR country ILIKE concar('%',?1,'%')" +
            "OR state ILIKE concar('%',?1,'%')" +
            "OR city ILIKE concar('%',?1,'%')", nativeQuery = true)
