...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    Optional<MovieCinema> findById(Long id); //optional, we need to find one object
    ...
    List<MovieCinema> findAllByMovie_NameContaining(String name); // here we are joining two tables

    //Write a derived query to list all movie cinemas in a specific location
    List<MovieCinema> findAllByCinema_Location_Name(String name); // joining 3 entities
